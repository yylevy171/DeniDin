...
- **Sanity suite** (`@pytest.mark.sanity`, Feature 059): a curated, cross-app, high-signal subset — every `sanity` test is ALSO `billed` or `expensive` and keeps that tier's rules unchanged (so `sanity` alone grants no run approval, and the `expensive` members still need per-run approval one at a time). `./scripts/run_sanity.sh` (repo root) runs the `morning-mcp-app` gate then the `denidin-app` billed subset **through `scripts/run_multiple_billed_tests.sh`** (stop-on-first-failure, live per-test sound-off); it never auto-runs the `expensive` members, only prints them as a manual checklist. Subset defined in two places (the decorators + `run_sanity.sh`'s arrays); `./scripts/verify_sanity_lists.sh` asserts they don't drift. Not a substitute for the full tiers, and not CI.
...
**Changelog**:
- v2.9.0 (2026-09-02): Feature 059 added the **sanity suite** (`@pytest.mark.sanity`) to §VII — a curated cross-app subset of the billed/expensive tiers, run together via `./scripts/run_sanity.sh` (through `run_multiple_billed_tests.sh`, live sound-off) as a fast end-to-end "anything obviously broken" check; expensive members are checklist-only, never auto-run. Not CI.
