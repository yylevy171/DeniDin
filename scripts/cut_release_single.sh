...
# anywhere - see scripts/deploy_release.sh (all apps at once) or scripts/deploy_release_single.sh
# (one app) for that (Feature 034, REQ-REL-001/005).
#
...
# scripts/lib/release_scripts_manifest.sh for the exact file list. Symmetric across every app
# (whichever is cut/deployed most recently is what keeps the scripts fresh on a real, non-git
# deploy directory like prod's) - see scripts/deploy_release.sh / scripts/deploy_release_single.sh
# for the unpack side.
#
# 2026-09-07 (bugfix-043 follow-up): renamed from cut_release.sh to cut_release_single.sh, same
# rename/reason as scripts/deploy_release_single.sh. scripts/cut_release.sh now means something
# different: it cuts a release for ALL apps at one shared version + summary, one call to this
# script per app - use that instead when cutting all apps together (the common case, e.g. this
# bugfix's own b43v3/b43v4 cuts). This script remains the right tool for cutting a single app on
# its own, or the rare case where apps genuinely need independent per-app summaries.
#
