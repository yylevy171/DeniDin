...
# Generated no-op override: the Windows box is the only place prod runs,
# so the base compose file's own paths are already canonical for it (same
# "root clone" pattern CLAUDE.md documents for the multi-clone case, just
# for this one dedicated machine). Static content, safe to regenerate
# identically on every deploy.
cat > "$STAGE_DIR/docker/docker-compose.prod.local.yml" <<'EOF'
# Generated by build_and_package.sh - no-op stub (this machine's own
# canonical paths already match docker-compose.prod.yml's defaults).
services: {}
EOF
