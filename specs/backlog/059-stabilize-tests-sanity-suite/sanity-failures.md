...
| S4 | `test_image_classification_e2e.py::test_personal_note_is_neither_bank_nor_agreement` | real model misclassification | ✅ **FIXED 2026-09-03 (user-approved)** — (a) `prompts/image_analysis.txt`: added two mandatory-classification bullets — a handwritten name+amounts note on notebook/notepad paper with no letterhead/terms/parties/signature is `unknown` (personal working note, not a fee agreement); and "when in doubt → `unknown`, let the system ask — asking is a virtue, a wrong guess is the failure". (b) test now fails only on a *confident, complete* real-bucket classification or an actual ledger event captured off the note — an incomplete flag or a clarifying question is accepted. Verified: passed once (model returned `doc_type='unknown'`); regression pass over the other 6 expensive sanity image/capture tests all green. |
