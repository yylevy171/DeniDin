...
- [ ] **T001** Add `mcp: Dict = field(default_factory=dict)` to `AppConfiguration`
...
- [ ] **T002** Implement `src/handlers/morning_mcp_locator.py` — `MorningMcpLocator(config)`
...
- [ ] **T003** [morning-mcp-app] Publish the status file: new `mcp.status_file` config field
...
- [ ] **T004a** [US1] Write the first failing E2E test (see **T010a**) — it is RED because no
  Responses/MCP reply path exists yet. **This is the RED gate for the whole reply slice.**
- [ ] **T004b** [US1] Migrate the **reply path** in `ai_handler.py`:
...
- [ ] **T005** Migrate **session summarization** (`ai_handler.py` transfer-to-long-term) to
  `responses.create` (no tools). Validated by existing/adapted expensive memory tests.
- [ ] **T006** Migrate **image/vision extraction** (`extractors/image_extractor.py`) to
...
- [ ] **T007** One-time §VIII-approved migration of any **pre-existing test** coupled to the
...
- [ ] **T008** Add the runtime-constitution tool section
...
- [ ] **T009** Implement `tests/expensive/denidin_mcp_e2e_helpers.py`: ephemeral ngrok tunnel
...
- [ ] **T010** [US1] `create_invoice`: positive (godfather webhook → invoice exists in sandbox)
  + negative (missing amount → AI asks, no malformed call).
- [ ] **T011** [US2] `list_invoices`: positive (seed 2 → list shows both) + negative (no match →
  readable "אין תוצאות").
- [ ] **T012** [US2] `get_invoice_details`: positive (seed → status/dates) + negative (bad id →
  friendly error).
- [ ] **T013** [US2] `update_invoice_status` paid: positive (mark paid → linked Receipt, status
  flips) + negative (reopen paid → graceful Hebrew rejection).
- [ ] **T014** [US2] `update_invoice_status` cancel: positive (cancel → linked Credit Invoice
  type 330 in sandbox) + negative (cancel nonexistent → friendly error).
- [ ] **T015** [US3] `add_client`: positive (add with valid ח.פ → client in sandbox) + negative
  (invalid tax id → friendly error; missing name → AI asks).
- [ ] **T016** [US2] `get_financial_summary`: positive (seed → totals/counts) + negative (custom
  period w/o dates → AI asks).
- [ ] **T017** [US2] `download_invoice_pdf`: positive (seed → real download URL) + negative (bad
  id → friendly error).
- [ ] **T018** [US4] RBAC-negative: client webhook invoicing prompt → no MCP tool attached →
  nothing created in sandbox → bot still replies.
- [ ] **T019** [US5] Scope-negative: godfather unrelated prompt (haiku) → no `mcp_call`.
- [ ] **T020** [US6] Graceful degrade: status file missing/stale → godfather invoicing prompt →
  normal reply, no crash, nothing created.
- [ ] **T021** Scenario A — full lifecycle: add client → create → list → cancel (multi-turn);
  verify client + original invoice + linked Credit Invoice in sandbox.
- [ ] **T022** Scenario B — conversational slot-filling: "צור חשבונית" (no details) → AI asks →
  supply → created; no premature/garbage call in between.
- [ ] **T023** Scenario C — confirm-before-act: create prompt → AI asks to confirm (no call) →
