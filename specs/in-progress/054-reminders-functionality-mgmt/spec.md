...
**Explicitly not solved by this feature**: yearly recurrence cadence; editing/cancelling a single
occurrence of a recurring series independently of the rest of the series (an edit or delete acts
on the whole series — see Assumptions); reminders that fire to a chat other than the creator's own
(no "set a reminder for someone else" capability, consistent with 013 being out of scope).
...
- **Occurrence status**: `pending` (not yet due, or due but not yet delivered) or `fired`
  (delivery to the user has completed). Set independently per occurrence.
...
- **Missed sweep window**: the delivery mechanism was not running (e.g. container restart) when
  ...
  delivery was late.
- **Deleting a recurring reminder mid-series**: all remaining `pending` occurrences are
  cancelled; occurrences already `fired` remain as historical records, untouched.
...
- **FR-010**: System MUST support modifying or deleting an existing reminder entirely through
  ...
  reminder (disambiguating among candidates if the description matches more than one) → for a
  modification, system guides the user through which field(s) to change → system requires the
  same explicit approval gate as creation (FR-003) before applying the change or deletion.
...
- **FR-012**: Deleting a recurring reminder MUST cancel all of its remaining `pending`
  occurrences; occurrences already `fired` MUST remain as unmodified historical records.
  Modifying a recurring reminder's recurrence rule or message text applies to the whole series
  (all remaining `pending` occurrences) — editing a single occurrence independently of the rest
  of its series is out of scope.
...
- **Occurrence**: One scheduled firing belonging to a Reminder — due datetime, status
  (pending/fired), and (once fired) the actual delivery timestamp.
...
- **Edit/delete granularity is whole-series, not single-occurrence**: unlike some calendar apps'
  "this occurrence only" vs. "this and following" vs. "entire series" distinction, this feature
  only supports whole-series modification/cancellation. Narrower per-occurrence editing can be a
...
- **SC-003**: A user can locate, modify, and cancel an existing reminder through natural
  conversation alone, including when their description is ambiguous, without needing to know or
  type any internal identifier.
