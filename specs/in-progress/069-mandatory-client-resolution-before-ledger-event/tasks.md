...
- **CONSTITUTION §V / ZERO MOCKING of internal components** — unit tests may mock **only**
  ...
  feature flag. **No feature flag and no config key exist for this feature** (2026-08-31 user
  direction — plan.md Complexity Tracking).
...
- **"Hard pointer"** — `event_datetime` is always derived from the **trigger message's own
  persisted `Message.timestamp`** (an Asia/Jerusalem ISO string), parsed and formatted
  `%d/%m/%Y %H:%M` (2026-09-02, option A — the Green API notification epoch is not persisted;
  no `local_from_timestamp` call on this path). **Never** `now_local()`, never the
  recognition-call clock, even after a multi-turn detour. Every hardcoded acceptance-test
...
- [ ] T009a [US1] Extend `tests/integration/test_ledger_client_resolution_routing.py` with the US1 assertions — an exact-match `הסכם` turn: the reply is produced by the main turn **independently** of recognition (force `recognize_ledger_event` to raise → the reply is byte-for-byte unchanged, SC-009); exactly one `LedgerEvent` after the turn against the exact Morning name; **zero** client-identity question in the reply (US6); the `recognized` + `written` breadcrumbs pair on `session`. OpenAI stubbed.
- [ ] T009b [US1] No new production code expected beyond Phase 2 — if T009a surfaces a decoupling gap, apply the minimal fix in `denidin.py` / `ai_handler.py`. Otherwise mark "covered by Phase 2". **BLOCKED until T009a is RED.**
- [ ] T010 [US1] ✅ Self-verify checkpoint (no user gate): decoupled exact-match `הסכם` text captures silently, one turn, reply unchanged — full billed proof is Phase 11 (T039).
...
- [ ] T017b [US4] Apply the T015-approved section to `config/runtime_constitution.md` (constitution hot-reloads by mtime — no code change). **BLOCKED until T015 approved (§XIX) AND T016a written + RED.**
...
- [ ] T024a [P] [US7] Extend `tests/unit/test_media_handler.py` — a recognized `בנק` **image** and a recognized `הסכם` **image** → a synthetic conversational turn is routed through the shared path with the right `text_content` (the stash) / `chat_id` / resolved `sender` / resolved `phone_number` (for RBAC) / `message_id` / **Green API notification timestamp** / `source_medium="image"`; `add_ledger_events_from_call` is **not** called on this path; an exception in routing → the existing friendly media-error notice, **nothing** persisted, **no** fallback to `add_ledger_events_from_call`; `analysis_result["ledger_events"]` empty → routed as an ordinary media turn (unchanged), no stash; Step 11 `_store_media_turn` still runs for the media artifact + caption. OpenAI mocked.
...
- [ ] **Checkpoint (no approval gate)**: confirm T023a + T024a + T025a are written, RED, and committed before the Task B items.
...
- [ ] T024b [US7] Rewire `src/handlers/media_handler.py` Step 10 (~lines 218–235) — **media-type-agnostic**: when `analysis_result["ledger_events"]` is non-empty and its `source_type` ∈ {`בנק`, `הסכם`}, build the stash and route a synthetic conversational turn (Feature 030 contact-card pattern) through the shared `_process_conversational_message` path via the `denidin.py` helper (T025b), deriving `source_medium` from the extractor (`ImageExtractor` → `"image"`), **instead of** calling `add_ledger_events_from_call`; on exception surface the friendly media-error notice and persist nothing (CONSTITUTION §XVIII), **no** fallback. Step 11 `_store_media_turn` unchanged.
- [ ] T025b [US7] In `denidin.py` `_process_media_message`: route the synthetic media-ledger turn through `_send_ai_response_and_attach(...)` and then `_run_post_turn_ledger_recognition(...)` (both from Phase 2's T007b) — so the media synthetic turn gets the interactive-button attach wiring **and** the same post-turn recognition hook as a typed turn. The conversational path must stay byte-for-byte identical (existing tests green).
...
- [ ] T028b [US10] In `src/handlers/media_handler.py`: recognize `DOCXExtractor`'s `document_analysis.document_type == "הסכם"` as the routing signal (reusing Phase 9's media-type-agnostic Step 10), derive `source_medium="document"` for `DOCXExtractor`. In `src/handlers/extractors/docx_extractor.py` (docstring ~lines 48–51) and `src/handlers/extractors/base.py` (comment ~lines 55–58): add a note that **ledger recognition is now post-turn (in `AIHandler.recognize_ledger_event`), for every medium** — the extractor does not run it. **No** `capture_ledger_events_from_text` call added. **BLOCKED until T028a is RED.**
