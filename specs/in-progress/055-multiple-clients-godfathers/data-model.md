...
| `data_root` | derived, not stored | `{environment_data_root}/{tenant_id}/` — sessions, memory, ledger events all live under this path. Never a config field itself; always derived from `tenant_id`, and inherently environment-specific even though identity isn't (dev and prod never share session/memory data for the same tenant). |
| Memory (ChromaDB) | derived, not stored | **Clarified 2026-08-17**: `{data_root}/{tenant_id}/memory/` is a genuinely separate ChromaDB persistent client/directory per tenant — not one shared ChromaDB instance with tenant-prefixed collection names. Collection names inside stay exactly as today (`memory_{entity_id}`, `_public`, `_private`, `memory_system_context`) — the directory boundary alone is the isolation mechanism, consistent with sessions/events, no extra name-prefixing needed. |
