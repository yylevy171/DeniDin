...
- [x] T090a [US4] `apps/rolling-memory-backfill/tests/billed/test_backfill_billed.py` — **1 passed** (2026-09-03). The real-OpenAI twin of T041a: one comprehensive billed test through the exact nightly-roll summariser path proving one `daily_summary` per non-empty pre-window (chat,date) with `source="migration"`, markers for every day incl. empty, same-args re-run is a true no-op (`billed_calls=0`, 0 new records, no dupes), a following real `_sweep_daily_roll` skips every migrated day, `assert_message_integrity` before + after, zero raw bytes touched. ~4 real summary calls on the first run. (Self-contained tmp `data_root` — does not need `morning-mcp-app-dev` or a live dev chat; the backfill path never attaches Morning MCP. The report is printed for review.)
- [x] T090b [US4] Ran the real backfill against **dev** (2026-09-04). **Dev-data cleanup first** (👤-approved): dev's `sessions/` held 1 active + 10 `expired/` dirs — 8 stale test sessions for `972522968679@c.us` (Feature 070 reconcile would pick the 240-msg one over today's live 22-msg one) + a test number + a 2-emoji group dir. Moved all 11 aside to `dev_data/_pre070_sessions_archive_20260903/` (kept, not deleted); promoted the real user+Ayala group session (`504112c0`, `120363410226011645@g.us`, 4 msgs) from `expired/2026-08-19/` to an active dir + cleared its stale `storage_path`. End state: exactly the 2 real persistent chats. Backfill (`--since 2026-08-19 --until 2026-08-21`): **1 summary** (group Aug 19, 4 msgs), 5 empty-day markers, **1 billed call**, integrity held.
- [x] T091 [US4] Post-run dev verification (2026-09-04): 1 `daily_summary` (group Aug 19, `source=migration`, `scope=PRIVATE`, real Hebrew text); 6 `roll_markers.db` rows all `committed`; `assert_message_integrity` clean for both chats. Dev rebuilt on `feature/070` (`v0.5.3`) + started: startup catch-up sweep ran on real data → **0 "Created new session"** (both chats mapped to their existing sessions), 12 `daily_summary` records total, 21 markers/chat all `committed`, no errors (the one `ERROR` is the pre-existing Feature-025 "143 candidate documents" guardrail on the dev Morning sandbox — unrelated).
- [x] T092 [US4] Dev continuity across a real `docker restart denidin-dev-denidin-app-dev-1` (2026-09-04): 2nd-boot startup roll sweep **fully idempotent — 1 second, 0 OpenAI calls, 0 new records, 0 "Created new session"**; roll scheduler restarts clean (02:00 Asia/Jerusalem). **Real WhatsApp turns then confirmed end-to-end (human sent both):** (1) group `120363410226011645@g.us` "זוכר שהזכרת לי משהו לפני שבועיים?" → routed to the promoted session `504112c0`, **no "Created new session"**, `Added 10 recalled memories to system prompt` (the collection's sole record is the backfilled Aug-19 `daily_summary`), model replied about a ~2-week-old reminder (chose the still-active weekly reminder via `list_reminders` over the one-time Aug-19 "קקי" summary — both valid readings of a deliberately vague question; the summary *was* in-context). (2) 1:1 `972522968679@c.us` "מה שלומך ומה גירסתך" → `Retrieved 22 messages from session history` / `Including 22 messages from conversation history` — the **full rolling window of the pre-existing active session**, all Aug-24→Sep-3 turns threaded in, **no "Created new session"**, correct answer ("גרסה 0.5.3"). Restart continuity + rolling-window + recall all confirmed on real migrated dev data.

**Checkpoint (Phase 9 done ✅ 2026-09-04)**: T090a green; the dev backfill ran once, is idempotent on
re-run (CLI re-run *and* the container restart's startup sweep), left every raw message byte-unchanged;
dev continuity across a real container restart confirmed; and two real inbound WhatsApp turns on the
migrated dev data confirmed session continuity (0 "Created new session"), the 22-message rolling window,
and daily-summary recall in-context. AC-4 satisfied.
...
- [x] T100 [US4] **Executed 2026-09-06** (👤-approved, one-phase per `MIGRATION-RUNBOOK-prod.md`). Prod stopped; `data/` pulled to `~/denidin-migration/prod-pull-20260906/` (backup #1, verified byte-perfect via md5 on `chroma.sqlite3`/`reminders.db`/`session.json`, then frozen read-only) → `cp -r` → `prod-work-20260906/`. Pipeline on the working copy: **consolidate** 99 dirs → 2 canonical (`12e158e2` 976 msgs, `83495a2b` 129 msgs), 0 dup, integrity PASS; **backfill** `--since 2026-08-01 --until 2026-08-23` → 22 `daily_summary` (15 group + 7 1:1), 46 markers committed, 22 billed calls; **finalize** archived 621 + 101 msgs (355 / 28 now live); **purge** 24,203→15 and 13→7 records (24,194 legacy `session_summary` deleted). `rsync --delete` push-back (box md5-verified). Deployed `0.5.4-70` (morning then denidin) via `scripts/deploy_release.sh`. Note: the runbook's box-side `cp -r` backup #0 could not be made (dir locked by stopped containers); backup #1 (`$PULL`) is the recovery point. rsync flag `--info=progress2` unsupported by the Mac's rsync — dropped.
- [x] T101 [US4] **Prod verification 2026-09-06 — PASS.** Startup: `SessionManager initialized (Feature 070: rolling window, no expiry)`, **0 "Created new session"**, **0 reconcile "maps to N dirs" warnings**, **0 ERROR / 0 Traceback**. Startup daily-roll sweep additively filled Aug 24–Sep 5 (12 more summaries; final total **24 group + 10 1:1**, 0 non-`daily_summary` records). Roll scheduler armed **02:00 Asia/Jerusalem**. `active_env.json` restored to the 2026-08-05 `active_envs` schema post-deploy (deploy writes the old scalar form, a watchdog no-op). Real WhatsApp turns (godfather 1:1): recent-context reply + **date-anchored recall** — *"על איזה לקוחה שאלתי אותך בתאריך 14.8?"* → `Added 10 recalled memories` / `Retrieved 30 messages` / correct named answer, no new session.
...
- [x] T060 [P] `pylint` 9.38/10 (> 7.0 gate) + `mypy` clean for **Feature 070 code** — `src/models/config.py:181` (the one mypy error the feature introduced) fixed with an explicit `Dict[str, Dict[str, Any]]` annotation on `memory_defaults`; the 6 remaining mypy errors are all pre-existing on `origin/master` (Features 033/044/054), verified by stashing. `apps/morning-mcp-app` unit suite 355 passed; its logger/time_utils changes are lint/type clean.
- [x] T061 [P] Full non-billed suite green — `apps/denidin-app` **1364 passed**, 169 deselected, 0 failures/errors. `apps/morning-mcp-app` **unit 355 passed**; its `tests/integration/test_morning_sandbox_*` suite is currently blocked by an environmental `403 Forbidden` from the Morning sandbox itself (HTML nginx rejection, not the API) — unrelated to Feature 070, which touches only `logger.py` / `time_utils.py` there.
...
- [x] T063 [P] `[haleluya]` Updated `CLAUDE.md` "Architecture" section (2026-09-06): new **Memory model (Feature 070)** paragraph; rewrote the `session_manager.py` / `memory_manager.py` bullets; replaced the `cleanup_service.py` bullet with `daily_summary_roll_service.py`; added `roll_marker_store.py` / `summarizer.py` bullets and folded `memory_collections.py` into the `memory_manager.py` bullet; updated the `denidin.py` (vestigial `cleanup_thread`, new roll scheduler) and `ai_handler.py` (removed `transfer_session_to_long_term_memory` / `recover_orphaned_sessions`) bullets and the message-flow block. One tracked `CLAUDE.md` file — the root clone picks it up on `git pull` after merge.
