...
## Status
Done — Merged to master (PR #163). Fix direction approved by human (Option C, 2026-08-03), scope extended to also cover the equivalent bug in `Bot.run_forever()` (same human approval — see "Fix Direction (Approved)" below). Implemented, tested (new unit tests + full existing suite, 586/586 passing), lint/mypy clean (no new issues vs. pre-fix baseline), and re-verified against the real, currently-affected dev Green API instance (`710722700313`) via `tests/integration/test_real_api_connectivity.py`. Real Windows-hosted `prod` deploy verification intentionally deferred (2026-08-03) — `denidin-dev` was running under another clone's ("Ruth") dev lock at merge time, so no test deploy was performed as part of this haleluya flow; a rebuild/redeploy is still needed before this fix is live anywhere.
