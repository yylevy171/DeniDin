...
- **Phase 11 fixtures, as built 2026-09-04:**
  - T031/T032: `agreement_new_client.txt`, `agreement_ambiguous.txt`, `agreement_doc_multi.docx`
    (+ `build_agreement_doc_multi.py`) authored with **explicit in-body agreement dates** so the
    manifest `txn_date` is deterministic (webhook `timestamp` = wall-clock `now`, so "היום"
    would not be).
  - T033/T034: the image manifests **reuse existing** `tests/fixtures/media/ledger_events/`
    files rather than adding new binaries where possible — `agreement_photo_multi` →
    `agreement_idan_shabtai.jpg`; `deposit_exact_match` → `Bank-test-image.jpg`;
    `deposit_zero_matches` → `bank_deposit_kehilat_tzair.jpg`. **One new image added by the
    operator**: `bank_transfer_grinfeld.jpg` (payer גרינפלד אורלי, ₪800, 23/08/2026, ref 3342,
    bank 12 / branch 153 / acct 282829) — drives **both** `deposit_one_partial` (US7b) and
    `deposit_two_plus` (US7c); the tests seed non-exact partial-match clients and assert only
    the final resolved `client_name` (shared image ⇒ candidate count not asserted).
  - T035: the sandbox roster is documented in each new test module's docstring +
    `acceptance-regression-map.md`; per-test seeds live in each manifest's `seed_clients`.
  - T036/T037: `assert_event_matches_manifest` / `_two_hop` normalise `"4000"`↔`4000` and
    ISO↔`DD/MM/YYYY`, so a manifest may state either form; `בנק` banking-triplet + `reference`
    are tolerated by the backward "no hallucination" check (inherently source-traced).
...
- [x] T030A Run `speckit.analyze` over `spec.md` + `user-stories.md` + `plan.md` + `research.md` + `data-model.md` + `contracts/` + `tasks.md`. Resolve any inconsistency **before** writing acceptance-test code. (Task id `T030A` is deliberately between Phase 10's T030 and Phase 11's T031 — it runs there.)
...
### Status 2026-09-04 — WRITTEN, not yet run

All Phase 11 fixtures, helpers, and the two NEW test files are authored and committed
(collect-clean, 15 tests, pylint 10.0). Feature 59 seeding blocker confirmed cleared
(PR #275, in-branch). The full regression surface is mapped in
[`acceptance-regression-map.md`](./acceptance-regression-map.md) — **168** pre-existing
...
- [x] T039 [P] [US1] Describe + write `test_us1_mechanism_move` — decoupled exact-match `הסכם` capture: one turn, one `הסכם` event vs the exact Morning name, **no** second reply round-trip, `recognized` + `written` breadcrumbs, exhaustive manifest match.
...
- [~] T047a (operator request 2026-09-04) Which — **if any** — of the new billed (`test_e2e_ledger_069_*_billed.py`) / expensive (`test_e2e_media_client_resolution.py`) tests carry `@pytest.mark.sanity`.
  - **Partly resolved 2026-09-06 (forced by the T052 rework):** deleting the 2 duplicate `test_ledger_event_capture_e2e.py` image tests would have dropped sanity's coverage of the "image → mandatory-resolution → ledger capture" path to zero, so their 2 slots moved to the fuller Batch 1 equivalents: **`test_us7a_deposit_image_zero_matches_new_client`** (bank image, new-client detour) and **`test_us9_photographed_multi_component_agreement`** (photographed agreement, new-client detour) are now `@pytest.mark.sanity`. Decorators + `scripts/run_sanity.sh` EXPENSIVE array updated; `./scripts/verify_sanity_lists.sh` green (49 sanity, lists match).
  - **Still open:** whether any *billed* Batch 1 test (candidate: the flagship US4 new-client detour) joins sanity. Deferred until Batch 1 has a full green run — revisit then.
...
- [ ] T052 Rework the **18** pre-existing `billed`/`expensive` ledger-**capture** tests (Tier 1 of [`acceptance-regression-map.md`](./acceptance-regression-map.md)) to the mandatory-client-resolution world (the mechanism move is transparent — they assert on disk outcomes; the mandatory-client rule is the deliberate behavior change): `tests/billed/test_ledger_event_capture_billed.py`, `tests/billed/test_ledger_event_capture_text_billed.py`, `tests/expensive/test_ledger_event_capture_e2e.py`, and the Group B reference-approval tests that now silently gain events. Each needs (1) the `live_morning_tunnel` fixture dependency (T038) and (2) **either** a seeded exact-match Morning client (cheapest — the silent path) **or** a scripted multi-turn resolution detour (`converse_until_ledger_events_captured` + `ClarificationAnswerBank`). **Category A is untouched** — `tests/billed/test_ledger_query_billed.py` (query) and `tests/billed/test_accounting_reconciliation_billed.py` (reconciliation sweep) legitimately make real OpenAI calls and don't exercise the 069 capture path; only `test_morning_question_answers_and_creates_no_ledger_event` there gains a dependency on the recognition call classifying a read-only Morning question as `none` (adjust its assertion if needed, no rewrite). **No test asserts `schema_version`'s value** — if a reworked test still does, remove that assertion (CLAUDE.md).
  - **`tests/expensive/test_ledger_event_capture_e2e.py` — static rework DONE 2026-09-06** (operator: "fix all the tests before running any of them"). 2 tests deleted (superseded by Batch 1 US9/US7a), F3/F4/F5 reworked in full (godfather + seeded exact-match client + `_drive_detour_until_captured` loop + live-tunnel fixture check; `event_datetime` assertion dropped; F4 3-round approval loop + JSON `_assert_no_open_invoice_for`; F5 KNOWN-FAILURE block removed). **Model-behaviour assertions are UNTUNED — first real run will likely need adjustment** (answer-bank keywords, `max_turns`, F5's `expected_count=6`). See `acceptance-regression-map.md` Tier 1.
  - **`test_e2e_ledger_069_*_billed.py` hardening DONE 2026-09-06** (Batch 1 itself): US2 `_send_turn_and_approve` → 3-round approval loop; `_ledger_069_acceptance.assert_event_matches_manifest` now treats `stated_name_for_store_anyway` as a store-anyway signal (fixes US8a's shared-manifest false failure); US5/US10 seed calls pass `ensure_exists` + their manifests gained `ensure_exists: true` (fixes US10's `ClientAlreadyExistsError`). **US2's `accounting_document_display_number is None`** on the last run points at a real `persist_recognized_event` synchronous-חשבונית gap — implementation work, NOT a test change; the assertion stays.
- [ ] T053 Re-run the reworked billed tests via `scripts/run_multiple_billed_tests.sh` (sound off each result live); request approval and re-run each reworked expensive test one at a time (STOP-on-failure each). **Not yet run — operator directed "fix all before running any."**
...
- [ ] T058 Move `specs/in-progress/069-mandatory-client-resolution-before-ledger-event/` → `specs/done/v0.7.0/069-mandatory-client-resolution-before-ledger-event/` (flat — `cut_release.sh` later sweeps it into a version folder) and fill the spec `Status` line — **only as part of the `haleluya` flow, which is NEVER run without explicit user instruction.**
...
- **No feature flag; ONE config key** (`ledger_recognition_context_window_hours`, decision #2,
  float default 1.0, DI, no env var) — otherwise per the 2026-08-31 user direction (plan.md
...
- **`event_datetime` hard pointer** — `הסכם`/`בנק` date from the **completing message**
  (`_message_epoch(session, completing_message_id)`, `session.message_ids[-1]`), `חשבונית`
  from the Morning doc datetime; never `now_local()` / the recognition-call clock, never
  `verdict["trigger_message_id"]` (informational only — decision #10). `Message.ledger_event_ids`
  also back-links from the completing message.
