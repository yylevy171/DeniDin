"""
Integration tests for bot.py initialization and startup.
...
class TestBotConfiguration:
    """Test that bot.py loads and validates configuration correctly."""

    def test_bot_file_exists(self):
        """Test that bot.py file exists and is executable."""
        bot_path = Path(__file__).parent.parent.parent / 'bot.py'
        assert bot_path.exists(), "bot.py should exist"
        assert os.access(bot_path, os.X_OK), "bot.py should be executable"
    ...
    def test_bot_imports_required_modules(self):
        ...
        for required_import in required_imports:
            assert required_import in content, f"bot.py should import: {required_import}"

    def test_bot_defines_handle_text_message_function(self):
        """Test that handle_text_message function is defined in bot.py."""
        bot_path = Path(__file__).parent.parent.parent / 'bot.py'
        with open(bot_path, 'r') as f:
            content = f.read()
        
        assert 'def handle_text_message' in content, "bot.py should define handle_text_message function"
        assert '@bot.router.message' in content, "bot.py should use @bot.router.message decorator"

    def test_bot_has_main_block(self):
        """Test that bot.py has main execution block."""
        bot_path = Path(__file__).parent.parent.parent / 'bot.py'
        with open(bot_path, 'r') as f:
            content = f.read()
