...
@pytest.mark.billed
def test_openai_asks_for_confirmation_on_client_name_variant_lookup(config, mcp_endpoint):
    """bugfix-039 regression guard, user-facing conversational check: when a
    user asks about an invoice using a name variant that isn't the client's
    literal stored spelling (a nickname/truncation - e.g. asking about
    "Yoss" for a client actually named "Yossef"), the model must call
    list_invoices and relay a genuine confirmation question naming the real
    resolved client back to the user - never silently claim no invoice
    exists (the live production incident this bugfix traces to, 2026-08-10),
    and never silently assume the guessed name is correct either.

    Complements test_list_invoices_tool_asks_for_confirmation_on_name_prefix_variant
    (tests/integration/) which checks the tool's own return string in
    isolation; this checks the full real conversational round-trip - an
    external OpenAI call actually relays that question intelligibly.
    """
    from openai import OpenAI

    server_url, auth_token = mcp_endpoint
    unique_marker = f"DENIDIN_BUG039_BILLED_{int(datetime.now(timezone.utc).timestamp())}"
    ...
    create_invoice(
        ...
        amount=63.0,
        description=f"Billed-test seed {unique_marker}",
    )
    # Give the sandbox's document-search index a moment (eventual
    # consistency, documented throughout this app's other real-sandbox
    # tests) before the OpenAI-driven lookup below.
    time.sleep(3)
    ...
    response = openai_client.responses.create(
        model=OPENAI_MODEL,
        instructions=OPENAI_ASSISTANT_INSTRUCTIONS,
        input=f"Was there ever an invoice issued for {queried_first_name} {unique_marker}?",
        tools=[
            {
                "type": "mcp",
                "server_label": "morning-invoices",
    ...
    mcp_calls = [item for item in response.output if getattr(item, "type", None) == "mcp_call"]
    list_calls = [call for call in mcp_calls if call.name == "list_invoices"]
    assert list_calls, f"Model did not call list_invoices. Full output: {response.output!r}"
    assert all(call.error is None for call in list_calls), (
        f"list_invoices call(s) reported an error: {[c.error for c in list_calls]}"
    )

    create_calls = [call for call in mcp_calls if call.name and call.name.startswith("create_")]
    assert not create_calls, (
        f"A lookup-only prompt must never trigger a create_* tool call: "
        f"{[c.name for c in create_calls]!r}"
    )

    assert any(client_name in (call.output or "") for call in list_calls), (
        f"list_invoices' own tool output should surface the real resolved client name "
        f"({client_name!r}) as a confirmation question, not silently proceed or refuse: "
        f"outputs={[c.output for c in list_calls]!r} arguments={[c.arguments for c in list_calls]!r}"
    )

    final_text = response.output_text or ""
    assert client_name in final_text or unique_marker in final_text, (
        f"Model's final reply should relay the real client name back to the user so they "
        f"can confirm/deny, not just silently report success or failure: {final_text!r}"
    )


@pytest.mark.billed
def test_openai_reports_no_invoice_when_client_truly_does_not_exist(config, mcp_endpoint):
    """bugfix-039 regression guard, user-facing conversational check: when a
    user asks about an invoice for a client name matching nothing at all
    (not even a plausible prefix/nickname variant), the model must call
    list_invoices and report that no invoice was found - never fabricate a
    document, and never ask an unanswerable confirmation question about a
    candidate that doesn't exist (user decision, 2026-08-10: "if truly no
    match on any prefix(es) - decide that there is no match").
    """
    from openai import OpenAI

    server_url, auth_token = mcp_endpoint
    nonexistent_name = "NO_SUCH_CLIENT_XXXXXXXX NO_SUCH_SURNAME_YYYYYYYY"
    ...
    response = openai_client.responses.create(
        model=OPENAI_MODEL,
        instructions=OPENAI_ASSISTANT_INSTRUCTIONS,
        input=f"Was there ever an invoice issued for {nonexistent_name}?",
        tools=[
            {
                "type": "mcp",
                "server_label": "morning-invoices",
    ...
    create_calls = [call for call in mcp_calls if call.name and call.name.startswith("create_")]
    assert not create_calls, (
        f"A lookup-only prompt for a nonexistent client must never trigger a create_* call: "
        f"{[c.name for c in create_calls]!r}"
    )

    assert any(
        (call.output or "") == "לא נמצאו חשבוניות התואמות את החיפוש." for call in list_calls
    ), (
        f"list_invoices should report the standard 'no results' message for a client name "
        f"matching nothing at all: {[c.output for c in list_calls]!r}"
    )
