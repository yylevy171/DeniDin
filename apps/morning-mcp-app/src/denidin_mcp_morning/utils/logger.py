...
Mirrors apps/denidin-app/src/utils/logger.py exactly (same setup_logger/
get_logger contract), so both apps behave identically for operators.

Logging Strategy:
- Production/Normal run: All logs go to logs/morning-mcp.log (default)
...
def setup_logger(
    ...
    Args:
        ...
        version_file: Path to the VERSION file to stamp onto every log line (Feature 034,
                      REQ-VER-003). Defaults to this app's real VERSION file; tests pass a
                      scratch path.
