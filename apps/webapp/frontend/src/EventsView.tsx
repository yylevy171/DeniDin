...
import { AuthError, EventRow, fetchContext, fetchEventDetail, fetchEvents } from "./api";
import {
  ChatPanel,
  ClientNameInput,
  DateRange,
...
export default function EventsView({
  ...
  const [dateFrom, setDateFrom] = useState(() => isoDaysAgo(settings.daysBack));
  ...
  const [applied, setApplied] = useState(() => ({
    ...
    client: "",
    global: "",
    from: isoDaysAgo(settings.daysBack),
    to: todayIso(),
  }));
  ...
  const [loadedDaysBack, setLoadedDaysBack] = useState<number>(settings.daysBack);
  ...
  useEffect(() => {
    load("load");
  }, [settings.daysBack]); // eslint-disable-line
  ...
  return (
    <View testID="events-view" style={{ flex: 1, backgroundColor: theme.bg }}>
      {/* action row: record count + refresh */}
      <View
        style={{
          flexDirection: "row",
          alignItems: "center",
