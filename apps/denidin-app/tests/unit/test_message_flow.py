...
class TestMessageFlow:
    ...
    def mock_openai_response(self):
        ...
        response = Mock()
        response.output_text = "Of course! I'd be happy to help you. What do you need assistance with?"
        response.usage = Mock()
    ...
    def test_openai_send_request(self, mock_openai_class, config, mock_notification):
        ...
        # Simulate what bot does: create OpenAI Responses API request
        from openai import OpenAI
        client = OpenAI(api_key=config.ai_api_key)

        # Prepare the request (as ai_handler.py does)
        # NOTE: Constitution content comes from AIHandler, not config
        instructions = "You are a helpful assistant."
        input_items = [{"role": "user", "content": message_text}]

        # Verify request structure
        assert input_items[0]['role'] == 'user'
        assert input_items[0]['content'] == 'Hello, can you help me?'
    ...
    def test_openai_receive_response(self, mock_openai_response):
        """
        Tests that the bot correctly receives and extracts the AI response from OpenAI's Responses API,
        including response text and token usage.
        """
        # Verify response structure
        assert hasattr(mock_openai_response, 'output_text')

        # Extract response (what bot does)
        response_text = mock_openai_response.output_text
    ...
    def test_complete_message_flow_integration(
        ...
        # Setup OpenAI mock (Responses API)
        mock_client = Mock()
        mock_responses = Mock()
        mock_responses.create.return_value = mock_openai_response
