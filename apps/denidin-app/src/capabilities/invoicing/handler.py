...
def propose_write(orchestrator, request: AIRequest, accumulated_context: str, note: str,
    ...
    if orchestrator.pending_approval_manager is None:
        return "Invoicing write path not configured."
    ...
    response = orchestrator.client.responses.create(
        ...
        instructions=orchestrator.build_instructions(
            CapabilityTag.INVOICING_WRITE, accumulated_context, request.timestamp,
        ),
...
def _resolve_approval(orchestrator, pending: PendingApproval, request: AIRequest) -> str:
    ...
    if executions > 1:
        ...
        return "⚠️ ייתכן שהפעולה בוצעה יותר מפעם אחת — יש לבדוק ידנית במורנינג."
    if executions == 0:
        logger.warning("Approval resolution produced zero executions of tool=%r", pending.tool_name)
        return getattr(response, "output_text", "") or "⚠️ הפעולה לא בוצעה."
    return getattr(response, "output_text", "") or "✅ הפעולה בוצעה."
