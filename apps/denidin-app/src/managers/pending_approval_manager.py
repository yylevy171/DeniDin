...
model wants to create in Morning (`create_invoice`, `update_invoice_status`)
but has not yet been authorized to execute. In-memory only (see class
...
class PendingApprovalManager:
    ...
    At most one pending approval is tracked per chat: only `create_invoice`
    and `update_invoice_status` are gated (Feature 022), and a single
    WhatsApp turn requesting two document-creating actions at once is an
    edge case where only the most recent pending request survives — not a
    silent-danger case, since nothing executes without approval either way.
    """
