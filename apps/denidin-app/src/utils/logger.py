...
`specs/done/v0.5.4-70/070-rolling-memory-window/contracts/logger-retention.md`.
...
**This module is byte-mirrored with `apps/morning-mcp-app/src/denidin_mcp_morning/
utils/logger.py` for the shared core (`_gzip_namer`, `_gzip_rotator`,
...
default, `DEFAULT_VERSION_FILE`, and morning-mcp's extra
`reconfigure_package_log_level` (not needed here). Keep the core identical.
...
def reconfigure_file_rotation(
    ...
    *,
    ...
    if existing is None:
        setup_logger(
            'denidin', logs_dir, log_filename, log_level,
