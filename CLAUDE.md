...
**Merging a code fix to `master` does not redeploy it.** `docker compose up -d`/`restart` does not rebuild on its own when source changed on disk — a running container keeps executing whatever image it was last built from. After merging any code change (not a mounted-data or per-env `config.<env>.json` change — those are picked up live/on restart; **everything else under `config/` — `runtime_constitution.md`, `ledger_recognition_prompt.md`, `fee_agreement_templates/` — is BAKED INTO THE IMAGE and is never bind-mounted (bugfix-066, 2026-09-26: a host-mounted copy silently overrode prod's baked constitution for weeks), so changing it needs a new build/release like any code change; only `config.<env>.json` is mounted, per environment, and it and the compose files are never overwritten by a release), rebuild and recreate every environment container currently running that app: `docker compose --project-directory . -f docker/docker-compose.<env>.yml build <service> && ... up -d <service>` (or the `run_*.sh <env>` script, but note it does not rebuild by itself either — build first). A merged RBAC fix once had zero effect on a running prod container for hours because of exactly this (2026-07-20). Note `/haleluya` itself never performs this rebuild/redeploy (see its definition below) — deploying, to any environment, is always a separate, explicit human-initiated request.
...
🚨 **AI agents: the bare invocations above are for human reference only — an AI agent may never run `pytest`/`make test` directly, for ANY tier, including unit/integration.** Use `scripts/run_unit_integration_tests.sh` (`apps/denidin-app/scripts/`, symlinked into `apps/morning-mcp-app/scripts/`) instead — same commands as above but through the wrapper, e.g. `scripts/run_unit_integration_tests.sh` (full suite), `scripts/run_unit_integration_tests.sh tests/unit/`, `scripts/run_unit_integration_tests.sh tests/unit/test_session_manager.py::test_function -xvs`. See CONSTITUTION.md §XIX / METHODOLOGY.md §XXIII ("No Bare `pytest` Invocations, Ever") — this rule is absolute, with no tier exceptions an agent may grant itself; billed/expensive already had their own wrappers below, this closes the one tier that didn't.
