...
- **`denidin-app` and `morning-mcp-app` are bundled at the hip within one
  ...
  ordering applies inside `scripts/deploy_release.sh` (bugfix-043,
  2026-09-07 — deploys ALL apps at one shared version in a single
  stop/start cycle; loads/retags `morning-mcp-app` before `denidin-app`).
  `scripts/deploy_release_single.sh` remains for deploying/rolling back one
  app on its own.
...
- **Never run a release-cutting or deploy script (`scripts/cut_release.sh`/`scripts/cut_release_single.sh`, `scripts/deploy_release.sh`/`scripts/deploy_release_single.sh`) on your own initiative, and never infer/guess a target version "the previous one" or "the one just cut" even when it seems obvious from context.** Every one of these scripts may only be invoked with (for the single-app variants) an app, or (for the all-apps variants) no app arg at all, plus — for deploy — an environment, and always an exact version explicitly supplied by the human in that specific request. `scripts/deploy_release.sh`/`scripts/deploy_release_single.sh` cover initial deploy to `dev`, promotion to `prod`, and rollback alike — same rule for all three (there is no separate "rollback script"). Approval for one release/deploy never carries over to the next — same rule as every other "ask every time" gate in this document. Every `deploy_release*.sh` call is **also, separately**, an environment-start action under the "NEVER START AN ENVIRONMENT... WITHOUT EXPLICIT APPROVAL" rule above (it recreates a running container in `dev` or `prod`) — both gates apply to every call; neither substitutes for the other, and cutting a version never by itself authorizes deploying it anywhere. **Once started, let it finish — never interrupt a `deploy_release*.sh` call mid-flight** (see "Versioning & Release Management" below for why: an interrupt between its stop and start steps can leave an environment down with its own recovery monitoring disabled and nothing else to catch it — a real prod incident, 2026-09-07).
...
## 🚨 AI AGENTS: LEDGER SCHEMA VERSION BUMPS ARE HUMAN-ONLY, EVERY TIME 🚨

Added 2026-08-26, after a real incident: `apps/denidin-app/src/managers/ledger_event_manager.py`'s `CURRENT_SCHEMA_VERSION` (stamped onto every persisted `LedgerEvent` as `schema_version`) was bumped 2→3 as a side effect of Feature 025 Phase 9's new fields (commit `0ed64ea`, 2026-08-23) with no accompanying human-approved decision record — only a stale inline comment left over from the *previous* bump, still describing the old rationale. The gap surfaced only later, by accident, during unrelated Feature 061 work. Reverted back to 2 (human decision, same day) once found.

- **`CURRENT_SCHEMA_VERSION` may only change on an explicit, fresh human decision — never as an incidental side effect of adding/changing fields in the same commit.** The same "ask every time, never infer from context" discipline as the version/release rule above applies here, even though this isn't an app *release* version — it's exactly as binding.
- **Every change to `CURRENT_SCHEMA_VERSION` must add a matching entry to `SCHEMA_VERSION_HISTORY`** (same file) in the *same* commit — never after the fact. `_verify_schema_version_history()` fails loudly at import time if the two ever drift apart; that check exists specifically because this incident happened once already with no such guard.
- **No test anywhere may assert on `schema_version`'s value — not a hardcoded literal, not even a comparison against `CURRENT_SCHEMA_VERSION` itself.** The constant is a governance decision, not application behavior a test should pin; a test asserting on it breaks for reasons unrelated to what it's meant to verify the moment the constant legitimately changes. (2026-08-26: every such assertion across the test suite was removed for exactly this reason — see `test_ledger_event_manager.py`'s `TestSchemaVersion`-removal comment for the coverage this traded away and how to restore it value-agnostically if ever needed.)

## 🚨 AI AGENTS: "SHOW ME THE FULL CONVERSATION" MEANS VERBATIM, BOTH SIDES, NOTHING ELSE 🚨

Added 2026-08-19, after the user had to ask **four times** in the same debugging session before getting what they actually asked for. When a human asks to see "the conversation," "the full conversation," or equivalent — from a test log, a session file, wherever — the response is **only** the literal turn-by-turn text both sides actually said, verbatim, in order, nothing paraphrased or dropped:

- **No interpretation, no analysis, no root-cause narrative mixed in.** Not "here's roughly what happened" — the exact words, every turn, both the human/user side and the assistant/bot side.
- **No silent substitution of an intermediate/logged value for what was actually sent.** A debug log line's `output_text=` field is not necessarily what the user received (an exception handler can replace it downstream) — find and use the value from the point it was actually transmitted (e.g. a "sending to user"/"would send to user" log line, not an earlier `_finalize_response`-style debug snapshot). Verify this distinction before presenting anything as "what was said," not after being corrected for getting it wrong.
- **No truncation.** A multi-line reply (real newlines in the message body) must be shown in full, not cut at the first line because of a naive single-line log-parsing regex.
- **Don't downgrade the request into something adjacent** — a table summarizing turns, a bulleted paraphrase, a "here's the gist" — even if that feels more efficient or more useful. If more analysis is also warranted, it goes in a clearly separate section *after* the verbatim transcript, never blended into it or offered instead of it.
- If it's genuinely unclear which conversation/log/timeframe is meant, ask — but once that's established, give the plain transcript first, before anything else.

## 🚨 AI AGENTS: EVERY NEW TOOL-BEARING FEATURE NEEDS EXPLICIT CONSTITUTION BOUNDARIES 🚨

Added 2026-08-19, after Feature 054 (reminders) shipped with reminder tools (`create_reminder`/`list_reminders`/`modify_reminder`/`delete_reminder`) attached to every GODFATHER/ADMIN turn but **zero mentions of reminders anywhere in `config/runtime_constitution.md`** — no scope, no "when NOT to use this," nothing. A real billed E2E test for an entirely unrelated feature (Morning client management) then showed the model repeatedly reaching for reminder tools when confused by an ambiguous reply mid-flow, including inventing a placeholder `reminder_id='unknown'` — because nothing told it reminders were out of scope there, or that it should ask instead of guessing. The user's framing: **"EVERYTHING IS BOUNDED, SO EVERYTHING NEEDS TO BE WELL-DEFINED in the runtime const. Not just vaguely defined or relying on the tool's hard definitions - it must be WELL-DEFINED."**

- **A tool's own JSON-schema `description` field is not enough.** It's sufficient for the model to know a tool exists and how to call it mechanically — proven by every "explicitly ask for a reminder" test passing cleanly — but it establishes no boundary against being reached for when the model is confused about something else entirely. Every tool-bearing feature needs an explicit `runtime_constitution.md` section stating: when it applies, when it explicitly does NOT apply, and that ambiguity is resolved by asking, never by picking whichever tool happens to be available.
- **Cross-reference in both directions.** Every *other* tool-bearing section must explicitly state that the new feature is out of scope for it too (mirroring the existing "Ledger Event Recognition" pattern's own "an Invoice Management action is automatically 'Neither'" bullet) — a one-way boundary (the new feature excludes others) isn't enough; the existing sections must exclude the new one back, or an ambiguous turn in *their* context can still misfire into it.
- **Short/ambiguous replies always answer the most recently pending question in the SAME context** ("כן", "לא", a bare name) — never a trigger for reinterpreting the turn as belonging to a different tool domain. This is now stated once, generally, in `runtime_constitution.md`'s "Contexts of Operation" section, and every domain-specific section should point back to it rather than re-deriving its own version.
- This applies structurally to *any* new tool family added to this bot going forward, not just reminders — check `runtime_constitution.md` for the new feature's own section, and for updated cross-references in every existing tool-bearing section, before considering a new tool-bearing feature done.

## 🚨 PRODUCTION INCIDENTS ARE NOT ACCEPTABLE — REBOOT/RESTART RECOVERY MUST BE VERIFIED END-TO-END, NOT JUST "CONTAINER IS UP" 🚨

Added 2026-08-25, after a real incident: the Windows prod box (Feature 035) took a routine, unattended Windows Update reboot overnight (`MoUsoCoreWorker.exe`, a planned OS restart — nothing anyone did). Feature 035's own reboot-recovery mechanism worked exactly as designed — both containers came back up automatically, `RestartCount=0`, no crash-loop. **But `morning-mcp-app-prod`'s startup code queried ngrok's local API for the tunnel's public URL exactly once, before the tunnel had actually finished establishing, got nothing back, and gave up — leaving the shared status file (`shared/mcp-status-prod/…json`) stuck at `status: "not running"` with no retry, forever.** `denidin-app-prod` read that same stale file on every turn and silently proceeded with Morning invoicing tools unavailable — a real, client-facing capability outage that went undetected for hours, until a human happened to ask "why is Morning connectivity broken." **Real clients were impacted. This is a production incident, full stop — not a "degraded gracefully" non-event.**

- **"Containers report Up" is not the same claim as "the service actually works."** `verify_windows_prod.sh`'s "PASS" on internal `/health` said nothing about whether the *external* tunnel handshake that the whole point of `morning-mcp-app` depends on had actually completed. Any reboot/restart-recovery verification (Feature 035's own, or any future equivalent) MUST validate the **full external dependency chain** for every service that has one — tunnel/handshake established, status file reflects the true live state, the *dependent* app (`denidin-app`) itself confirms the tool is attached — not just that a process is running inside a container.
- **No startup-time external handshake may be a single one-shot check.** A local API poll, a tunnel-establishment check, a dependency-discovery read — anything that can plausibly still be initializing at the moment it's checked — MUST retry with bounded backoff before ever writing (or leaving) a failure/unavailable status. A single failed check followed by silent give-up, with nothing ever correcting the record afterward, is a bug wherever it appears in this codebase, not acceptable "best effort" behavior. See CONSTITUTION.md §XVIII for the binding technical rule this incident produced. **The specific fix first proposed for this (2026-08-25) was implemented and merged without the human's approval and was then reverted at the human's explicit direction as not the right approach — the rule below stands on its own regardless; a correct fix for the ngrok/status-file handshake specifically is still outstanding.**
- **A restart is a mitigation, never a fix.** Restarting prod to restore service (as was done here, with explicit human approval, per the "NEVER START AN ENVIRONMENT... WITHOUT EXPLICIT APPROVAL" rule above) closes the *symptom* for right now. It does NOT close the incident — the underlying race condition that caused it is still live and will recur on the next reboot until a real Bug-Driven Development fix lands (root cause → human approval → test-gap analysis → failing test → human approval → minimal fix → verify, per METHODOLOGY.md §VII). See METHODOLOGY.md §XXII for the mandatory follow-through this now requires.
- **Zero-incident is the standard this project holds itself to.** That doesn't mean incidents can never happen — it means every one that does gets a real root-cause fix, not just a restart and a shrug, and every reboot/restart-recovery mechanism this project builds must be verified against the *actual* external dependency chain, not just "did the process start."
...
- 🚨 **Deploying a cut release is done ONLY by running `scripts/deploy_release.sh`/`scripts/deploy_release_single.sh` directly — no manual pre-flight checks, no ad-hoc connectivity probes (`tailscale status`, manual `ssh`, etc.) beforehand.** The script performs its own connectivity/health verification internally and fails loudly with a clear error if something's wrong — a manual check first is redundant busywork, not caution, and the user has explicitly objected to it ("use the goddamn scripts"). This applies to `dev` and `prod` alike; the read-only Windows-prod operational scripts (`scripts/windows_prod/*.sh`) remain fine to use for their own documented purpose (checking status/logs), just not as a substitute pre-check before a deploy.
- 🚨 **A `deploy_release*.sh` invocation must be allowed to run to completion once started — never interrupted/killed mid-flight.** Its remote/prod path calls `stop_env.sh` (which stops both apps' containers AND disables the health-monitoring scheduled task) before `run_env.sh` (which restarts them and re-enables the task) — an interrupt landing between those two steps leaves prod fully down with its own auto-recovery monitor disabled, and nothing else will catch or fix it. This happened for real, 2026-09-07: two overlapping/interrupted single-app prod deploy calls for the same app left prod with zero running containers for ~41 minutes, undetected, because the very mechanism that would have noticed (the scheduled prober) had itself been disabled by the first call's `stop_env.sh` and never re-enabled. If a deploy call must be stopped once it has started, follow up immediately by running `scripts/run_env.sh <env>` to restore the environment and its monitoring — don't just move on.
...
🚨 **Mandatory (2026-08-18, `apps/denidin-app` only for now): run every single billed/expensive test through `scripts/run_single_test.sh <node_id>` — never a bare `pytest ... | tail`/`| grep`/`| head` invocation.** A raw pipe truncates pytest's output *before* it's ever seen — real incident, same session: an assertion/traceback got silently discarded by a `| tail -15`, leading to a wrong "can't tell what failed" report, followed by a second unapproved rerun just to see the part that was cut off. The script writes pytest's **complete**, untruncated output straight to `logs/test_logs/pytest_results/<test>_<timestamp>.txt` — nothing upstream of that file ever filters it — and leaves the app's own per-test-file log (`logs/test_logs/{test_file}.log`) completely untouched, exactly as it always was. Marker (`billed`/`expensive`) auto-detects from the node id's path. For a **sequence** of billed tests with stop-on-first-failure behavior, use `scripts/run_multiple_billed_tests.sh <node_id> [<node_id> ...]` (billed only — never for expensive, which requires its own separate approval per test, no batching, ever). **Neither script grants approval to run anything** — every invocation still needs its own fresh, explicit human go-ahead, same as before these scripts existed; they only fix *how output is captured*, not any approval gate.

🚨🚨 **INDIVIDUAL PER-TEST SOUND-OFF IS THE STANDARD FOR EVERY TEST RUN — ALWAYS, NO EXCEPTIONS, NEVER NEEDS TO BE ASKED FOR.** The moment *any* test's result is determined — pass, fail, error, or skip — relay it to the user as its own line, immediately, as it lands. This applies to **every** way tests are run: `scripts/run_single_test.sh` (in a loop or once), `scripts/run_multiple_billed_tests.sh`, `scripts/run_parallel_tests.sh`, `scripts/run_sanity.sh`, `scripts/run_sanity_parallel.sh`, a bare `pytest`/`make test`, unit, integration, billed, expensive — all of it. Both apps' `conftest.py` now emit a `>>> TEST [k/N] STATUS: <nodeid>` line per test **by default** (opt out only with `DENIDIN_TEST_SOUNDOFF=0`, which you never do); your job is to surface each of those lines to the user the instant it appears, not to collect them into one end-of-run summary. **Never** background a test run behind anything that buffers until the process exits (`| tail -N`, `| grep`, `| head`, a `$(...)` capture, an unfiltered redirect you only read at the end) and report only once everything is done — use a `Monitor`/`tail -f` on the `>>> TEST` lines (or poll the per-test files under `logs/test_logs/pytest_results/`) and speak each result out loud as it completes. Real incidents (2026-08-26, 2026-09-04): agents repeatedly ran multi-test sweeps with nothing streaming for minutes until the user had to demand a status in all caps. This is not a per-run preference to be re-confirmed — it is the permanent default, enforced in the scripts and the `conftest.py` hooks, and documented in METHODOLOGY.md §VI and CONSTITUTION.md §V/§VII.

- **`billed` tests** (`tests/billed/`, marked `@pytest.mark.billed`) make real, **text-only** OpenAI calls (chat completions, MCP tool-call turns) — cheap per run. **They can be run freely: no per-run approval needed, no one-at-a-time restriction, no log-reading requirement.** 🚨 **Do NOT stop to ask before running a `billed` test — the approval gate below is `expensive`-only and does not apply to `billed` at all.** Run with `scripts/run_single_test.sh "tests/billed/test_foo.py::TestClass::test_method"` (single test) or `scripts/run_multiple_billed_tests.sh <node_id> ...` (a sequence, stop-on-fail).
- **Expensive tests** (`tests/expensive/`, marked `@pytest.mark.expensive`) make real **vision/image/PDF/DOCX** OpenAI calls — meaningfully costlier (multiple sequential calls per test, e.g. `PDFExtractor` delegating to `ImageExtractor` per page) — and keep the full strict discipline below, unchanged. Don't run them repeatedly — read `logs/test_logs/` (and, for the specific run in question, its `pytest_results/` file) for a prior run's output before re-running, and only re-run after a code change you're confident fixes the issue.
...
- **Never run expensive tests all together.** Go one at a time via `scripts/run_single_test.sh "tests/expensive/test_X.py::test_name"`, never a bare `-m expensive` sweep and never `scripts/run_multiple_billed_tests.sh` (billed-only, see above).
- **Read existing logs in `logs/test_logs/` (and that run's `logs/test_logs/pytest_results/*.txt` file, if run via the script) before re-running anything.** A prior run's log may already answer the question.
...
- **`./scripts/run_sanity.sh`** (repo root) is the runner. It runs the `morning-mcp-app` gate test first (a broken tunnel/remote-MCP/Morning-sandbox path fails everything downstream), then the `denidin-app` billed sanity subset — **one test at a time, only ever through `scripts/run_single_test.sh`** (it never calls `pytest` itself and never batches a bare `-m` marker). Every result is sounded off live (`>>> SANITY [k/N] PASSED|FAILED|SKIP: <nodeid>` + a grep-friendly `SANITY-PROGRESS` line) — run it in the foreground or `tee` it, don't background it behind a buffering pipe. Stop-on-first-failure. `expensive` sanity tests are **never** auto-run — the script prints them as a copy-paste checklist to run by hand, one at a time, with fresh approval each, per the `expensive` rules above.
- **`./scripts/run_sanity_parallel.sh`** (repo root, Feature 075) is a faster, separate entrypoint for the same billed sanity subset — `pytest -n <N> --dist loadfile` (default `-n 6`; file-level parallelism, so each file's deliberate within-file ordering stays serial). It does **not** touch `run_sanity.sh`'s behavior. Runs the gate first, then one initial round + (by default) one retry round that re-runs **only** failures whose traceback carries an ngrok/MCP infra signature (`scripts/_sanity_retryable_failures.py` classifies; real assertion/model failures are terminal). `--no-retry`, `--retry-max-rounds N`, `--retry-delay`, `--retry-factor` tune it. Same `>>> TEST [k/N]` live per-test sound-off (`conftest.py`'s controller hook, on by default now — see the sound-off banner above) — run it in the foreground and relay each line as it lands, never behind a buffering pipe. Trailing sanity node-id args run just those in one parallel round (subset mode; gate skipped). Per-xdist-worker `test_data/<worker>/` isolation is `tests/e2e_helpers.py::sanity_worker_data_root()` — every sanity-subset `config` fixture (billed **and** the 2 expensive sanity files) sources `config.data_root` from it. `expensive` sanity tests are still never auto-run here.
- **Progress is tracked and resumable.** Each test's state (`pending`/`passed`/`failed`) is written to `apps/denidin-app/logs/test_logs/sanity_state.tsv` after every result; re-running skips anything already `passed` and picks up where it stopped. `--fresh` wipes state, `--status` prints the table, `--include-passed` re-runs green ones, `--mark <status> <nodeid>` hand-sets one row (use it to record an `expensive` test you ran yourself).
...
- **Israel local time everywhere** (bugfix-037, 2026-08-10 — replaced the old UTC-everywhere rule; there is no UTC in this codebase any more) — always `now_local()` from each app's `utils/time_utils.py`, never bare `datetime.now()` and never `datetime.now(timezone.utc)`. Every datetime is timezone-aware `Asia/Jerusalem`, so every persisted ISO timestamp and every log line carries a real offset (`+03:00`/`+02:00`). Values written before that date keep `+00:00` and still compare correctly (both sides are aware); nothing was migrated.
...
- **Feature flags are opt-in, not automatic** (CONSTITUTION §VI, revised 2026-09-14) — don't add one to new work unilaterally; ask the human whether this specific change warrants flag-gating (large regression risk, no other rollback path) before adding it. When one is used: lives under `config.feature_flags`, default `false`, disabled path byte-for-byte identical to before, removed once the feature is stable. Unit tests may set flags; integration tests must never set flags (they test default production behavior).
...
DeniDin is a WhatsApp bot (Green API) that forwards messages to OpenAI (`config.ai_model`, `gpt-5.6-luna` in dev/prod) with a two-part memory system, RBAC, and media processing. Full diagrammed architecture: `.github/ARCHITECTURE.md`.

**Memory model (Feature 070, shipped v0.5.4-70, prod-migrated 2026-09-06)**: one **long-lived `Session` per chat** — never expires, never recreated (`chat_index.db`, a SQLite `chat → session_id` map, is the authoritative lookup). Each turn's conversation context is a **rolling 14-day verbatim window** (`SessionManager.get_rolling_window`, capped per-turn by the acting role's `max_tokens`, reads only the live `messages/` dir — never `archived/`). A **nightly 02:00 Israel-local roll** (`services/daily_summary_roll_service.py`, `CronTrigger`) summarizes each day that has aged out of the window into **one ChromaDB `daily_summary` record per (chat, date)**, then physically `Path.rename`s those messages into `{session_dir}/archived/` (never `unlink`). A claim-first SQLite `RollMarkerStore` (`memory_rolls/roll_markers.db`, `PRIMARY KEY(chat, date)`, `claimed`→`committed`) makes the roll idempotent and crash-safe; a startup catch-up sweep fills any missed days. This **replaces** the pre-070 model (24h session expiry + `SessionCleanupThread` hourly re-summarization + `data/sessions/expired/` archival) — `services/cleanup_service.py` and `AIHandler.transfer_session_to_long_term_memory`/`recover_orphaned_sessions` are **deleted**. Known carried-forward limitation: `daily_summary` recall still rides `MemoryManager.recall()`'s pure embedding-similarity search, which ranks a date-specific summary poorly — redesign tracked as Feature 077.
...
Green API webhook → denidin.py @bot.router.message(type_message=...) handlers
  ...
  → SessionManager (resolve the chat's one long-lived session via chat_index.db, append message)
  → AIHandler
      → SessionManager.get_rolling_window() (last 14 days verbatim, oldest-first, ≤ role max_tokens)
      → MemoryManager.recall() (ChromaDB semantic search over daily_summary records, if enable_memory_system)
      → builds system prompt (constitution + role context + memories + rolling window)
...
**Interactive approval buttons (Feature 047)**: the Feature 022 document-creation approval gate now has two entry points into the same `PendingApprovalManager` state — the original typed reply (`"כן"`/`"אישור"`/etc., unchanged) and a real WhatsApp interactive-buttons tap (`"כן"`/`"לא"`, matching the approval prompt's actual closed question verbatim). When `AIHandler` creates a new pending approval, it also sets `AIResponse.offer_approval_buttons=True`; `WhatsAppHandler.send_response()` then sends via `sendInteractiveButtons` (through the library's `notification.answer_with_interactive_buttons` convenience method) instead of plain text, returning the sent `idMessage`, which `denidin.py` attaches to the pending approval (`PendingApprovalManager.attach_sent_message_id`) as `PendingApproval.sent_message_id`. A tap arrives as its own webhook type, `interactiveButtonsResponse` — routed via a new `@bot.router.message(type_message="interactiveButtonsResponse")` handler (**not** the library's `router.buttons()`, which only matches older deprecated button types and would silently never fire for this traffic) — carrying `selectedId` (the tapped button's stable identifier, `"denidin_approve"`/`"denidin_decline"`, never the display label) and `stanzaId` (linking back to the original message). `AIHandler.resolve_button_tap` treats a tap as stale — silently ignoring it, no reply at all — unless `stanzaId` exactly matches the pending approval's `sent_message_id`; this guard exists because `PendingApprovalManager` tracks only one pending approval per chat, so without it a stale tap on a since-superseded approval could incorrectly resolve the wrong one. WhatsApp/Green API itself imposes no staleness protection (confirmed live: a second tap on an already-resolved message produces an ordinary, unblocked notification) and offers no way to disable/grey out a button after the fact (`serviceMethods.editMessage` does not affect `interactiveButtons` messages, confirmed live) — the guard is entirely application-side. A live tap resolves by delegating to the existing `get_response`/`_resolve_pending_approval` pipeline via a synthetic `"כן"`/`"לא"` `AIRequest`, so a button decline behaves byte-for-byte like a typed one (including falling through to a fresh conversational turn). A `sendInteractiveButtons` failure surfaces a distinct error notice rather than silently falling back to the plain-text prompt.

**Reminders (Feature 054)**: RBAC-gated (GODFATHER/ADMIN only) natural-conversation reminder creation/listing/modification/deletion, one-time or recurring. Four local `type: "function"` tools (`create_reminder`/`list_reminders`/`modify_reminder`/`delete_reminder`, `ai_handler.py`) — `list_reminders` dispatches immediately (read-only, no approval needed); the other three create a `PendingLocalToolApproval` (`managers/pending_local_tool_approval_manager.py`, parallel to `PendingApprovalManager` but for local function-tool calls rather than MCP), reusing the same typed-reply/button-tap approval UX as Feature 022/047 — `get_response()`/`resolve_button_tap()` both check the MCP-pending manager first, then the local-tool one, deterministic order, at most one populated per chat in practice. Storage is `managers/reminder_manager.py`, a small SQLite database (`{data_root}/reminders/reminders.db`) whose schema is genuine RFC5545 iCalendar data — a master VEVENT (`rrule`/`dtstart`/`UID`) plus per-occurrence override VEVENTs (`reminder_exceptions`, keyed by a real `RECURRENCE-ID`) for single-occurrence edits/deletes — resolved into concrete due occurrences via the `icalendar`/`recurring_ical_events` libraries, never hand-rolled RRULE math. `services/reminder_delivery_service.py` runs one shared `APScheduler` `BackgroundScheduler` job (`CronTrigger(minute='*/5')`, wall-clock-aligned) that queries `get_due_occurrences` and delivers via `send_proactive_message` (`utils/green_api_bot.py` — an unprompted `bot.api.sending.sendMessage` call, Gate Zero-verified live 2026-08-19, see `specs/done/v0.5.0/054-.../research.md`); `run_startup_reminder_sweep` (24h lookback) catches up on boot, the periodic tick uses a narrower 1h lookback to cap blast radius if `fired_occurrences` bookkeeping is ever wrong. Delivery target is `delivery_chat_id` — the chat/group the reminder was created from, stored per-reminder at creation time — with a one-shot fallback to the reminder's actual creator's own 1:1 chat (`created_by_phone`, not a fixed godfather identity) if that send fails; neither is ever re-derived or persisted-back after the fact. A reminder is marked `status='completed'` (distinct from `active`/`cancelled`) once no future occurrence remains, so a finished one-time or count/until-bounded recurring reminder stops appearing as active without an unbounded table scan. `config/runtime_constitution.md`'s "Reminder Management" section defines when this tool family applies/doesn't (see METHODOLOGY.md §XXI) — reminder tools are RBAC-gated but not conversation-topic-gated, so explicit negative scoping is what keeps them from being reached for during an unrelated confused turn.

**Standalone Receipts & Transaction Account Cancellation (Feature 056)**: two related Morning accounting-rule changes for money that isn't ordinary invoiced income. (1) `create_receipt` (`apps/morning-mcp-app`) now also accepts a **standalone** call — `original_internal_morning_id=None` — for recording a receipt (type 400) with no linked invoice at all (e.g. a refundable deposit): resolves the client, builds a payload with no VAT line and a free-text description, no auto-linking to any other document (REQ-INV-014/015/018). (2) A new MCP tool, `cancel_transaction_account(original_internal_morning_id)`, cancels an open transaction account (`חשבון עסקה`, Morning type 300 only — any other type raises) by calling `client.close_invoice` directly — **no document of any kind is ever created** as a side effect (REQ-INV-020), and the reply never claims the account was "paid" (REQ-INV-026, `formatters.format_transaction_account_cancelled` deliberately bypasses the shared `translate_status` path). App-side idempotency guard (REQ-INV-024): if the account is already non-open (`status != 0`) when cancellation is requested, it's a no-op that returns the same confirmation without calling Morning again. Both are Group B "reference tools" (bugfix-038's category — approval-prompt data comes from a fresh, same-turn `get_invoice_details` lookup, never from the model's own memory of the original call) — `_GROUP_B_REFERENCE_TOOLS`/`_build_pending_approval_details`/`_build_pending_approval_fallback_text` (`ai_handler.py`) all have a dedicated branch for `cancel_transaction_account` so its approval prompt names the real account/client instead of falling through to the fully generic "there's a pending action" text (a real gap found via manual QA, 2026-08-20, fixed same day). A later, separate `get_invoice_details` lookup on an already-cancelled account can still describe it as "paid" (Morning's own status code 2 means either a real payment or this cancellation, and nothing persists which one happened) — a known, accepted, explicitly out-of-scope gap tracked as `specs/low-priority/058-morning-docs-calculation-nuances`.

**Accounting Document Reconciliation (Feature 025)**: closes the gap for Morning documents created *directly in Morning* (e.g. by hand, or by another system) with no corresponding DeniDin conversation, so the ledger doesn't silently miss them. `services/accounting_reconciliation_service.py` runs one shared `APScheduler` `BackgroundScheduler` job (`IntervalTrigger`, not `CronTrigger` — see the module's own docstring for why this differs from `reminder_delivery_service.py`'s shape) that periodically calls `_sweep_accounting_documents`: a real, minimal, standalone OpenAI Responses API call — Morning MCP tools + `LEDGER_EVENT_TOOL` attached, explicitly no session/`chat_id`/`AIRequest` — using a dedicated reconciliation prompt (`_build_reconciliation_prompt`) that lists recent Morning documents and asks the model to capture any not already in the ledger. `run_startup_accounting_reconciliation_sweep` does the same on boot for catch-up. Unlike reminder delivery, this sweep is entirely silent — no WhatsApp message is ever sent; its only output is persisted `LedgerEvent` files (`managers/ledger_event_manager.py`, extended this feature with `schema_version=2` and four new `accounting_document_*` fields — `accounting_document_display_number`/`accounting_document_type`/`accounting_document_client_name`/`accounting_document_creation_timestamp`), discoverable the same way any other ledger event already is (see Feature 033 above). Dedup is an in-memory tri-state (new/duplicate/anomaly) cache inside `LedgerEventManager` — never a per-tick disk re-scan, never in `ai_handler.py` — and a safety cap (5 days / 100 docs, whichever binds first) skips the entire tick on breach, checked by the service directly (non-AI) before ever calling OpenAI. Activation is config-gated: `config.accounting_ledger_update_freq` (minutes; `0` = inactive) — `60` in `prod`/`dev`/`test` alike (2026-09-01: prod was flipped on from its prior `0` once Feature 062's real backfill from `2025-09-01` landed and was verified live — see `specs/done/v0.5.4/062-prod-morning-ledger-backfill-completion/spec.md` for the backfill itself; the "nothing populates the ledger for documents Morning already held before the scheduler's first tick" gap this default used to guard against is now closed for that window). `apps/morning-mcp-app` gained one supporting change: `get_invoice_details`/`list_invoices` now also expose Morning's raw `creationDate` as `creation_timestamp`, plus an opt-in `format="json"` response shape for these two tools (Phase 9a) so the reconciliation prompt can parse results deterministically rather than relying on the model to transcribe a natural-language summary — widening `format="json"` to the rest of the read tools is deliberately out of scope here (`specs/in-progress/025-morning-sourced-ledger-events/tasks.md` Phase 9b, explicit user commitment, tracked but not yet done).

**Dynamic Capability Backbone (Feature 063, flag-gated, `feature_flags.enable_capability_backbone`, default `false`)**: a fully parallel, standalone orchestrator implementation of the message pipeline, selected once at `denidin.py`'s `initialize_app` startup — `handlers/ai_handler.py`/`config/runtime_constitution.md`/`config/ledger_recognition_prompt.md`/`prompts/*.txt`/`handlers/extractors/*.py` stay byte-for-byte untouched for as long as the flag exists. New code lives at `src/backbone/` (`orchestrator.py`'s `BackboneOrchestrator` — Intent Identification → Planning → per-step capability execution loop, reusing `ai_handler.py`'s existing followup-call pattern conceptually but as independent new code; `capability_tags.py`'s 9-value `CapabilityTag`; `intent_identification.py`/`planning.py`'s meta-capabilities) and `src/capabilities/` (4 domain subpackages — `invoicing/`, `ledger_events/`, `reminders/`, `media_analysis/` — each wrapping the same unmodified managers/extractors `ai_handler.py` already uses). New prompt files live under `config/prompts/` (`backbone.md` + `capabilities/*.md`), authored fresh, not a split of the legacy files. Read/query-side capabilities and one full write-approval template (Reminders — Write) are implemented; the remaining write-approval-flow parity (Invoicing — Write, Ledger Events — Capture, reminder modify/delete) and the raw-media-bytes threading from `denidin.py`'s dispatch into the orchestrator are tracked as follow-up work — see `specs/repo/features/063-refactor-oversized-handlers/tasks.md`'s "Deferred" section for the exact list. Full design: `specs/repo/features/063-refactor-oversized-handlers/`.

### Key components (`apps/denidin-app/src/`)
- **`denidin.py`** (repo root of the app, not under `src/`) — entry point; owns `denidin_app` (a `DeniDin` instance holding `ai_handler`, `config`, `whatsapp_handler`, `reminder_scheduler`, `daily_roll_scheduler`; `cleanup_thread` is a vestigial `None` param kept for signature stability post-Feature-070, only `.stop()`-ed if present, never constructed); registers all message-type handlers via a `HANDLER_REGISTRY`/`dispatch_notification` table (Feature 043 — no longer decorator-registered against a module-level `bot`, since a `GreenAPIMessageSource` now constructs the live bot inside `__main__` rather than at import time); `initialize_app(config_dict, green_api=None)` is the shared bootstrap used by `__main__`, every test, and the Feature 043 WhatsApp-export player (constructs `AIHandler` → `WhatsAppHandler` → `MediaHandler`, wires the reminder delivery scheduler + the Feature 070 daily-summary roll scheduler + its startup catch-up sweep).
...
- **`handlers/ai_handler.py`** — OpenAI integration via the Responses API, system-prompt construction, memory recall integration, rolling-window assembly (`SessionManager.get_rolling_window`), RBAC-aware token limits, current-date injection into `instructions`, and Morning MCP remote-tool attachment for godfather/admin roles (see "Morning MCP integration" below). (Feature 070 removed the old `transfer_session_to_long_term_memory` / `recover_orphaned_sessions` methods — long-term memory is now written only by the nightly daily-summary roll.) `instructions` is assembled in a fixed order — constitution text, then recalled memory context (appended to the constitution string), then a `---` separator, then today's Israel-local date computed fresh per call (`ai_handler.py:363-368`) — never templated into the constitution file itself. This ordering is also what makes the constitution (`config/runtime_constitution.md` — a single file shared identically by dev/prod/test as of 2026-07-23, not per-environment data; ~4.0K tokens as of 2026-07-23, measured via `tiktoken`'s `o200k_base`) eligible for OpenAI's automatic prompt caching: it's the stable, byte-identical prefix of every call (everything that varies — memories, date — comes after it), and OpenAI caches on the longest identical prefix for prompts ≥1024 tokens at a 50% discount on the cached portion, with no code changes required. Keeping any per-call-dynamic content appended *after* the constitution (not prepended or interleaved) preserves this.
...
- **`managers/session_manager.py`** (Feature 070) — one long-lived `Session` per chat under `data/sessions/<uuid>/` (`session.json` + `messages/*.json` + `archived/*.json`), never expires or recreates. `chat_index.db` (SQLite `chat_sessions(chat PK, session_id, updated_at)`) is the authoritative chat→session map. `get_rolling_window(chat, *, now=None, window_days=14, max_tokens=None)` is the per-turn context: every message whose Israel-local calendar date is within the last 14 days, verbatim, oldest-first, trimmed from the oldest until it fits `max_tokens` (read-only — nothing moved); reads **only** `messages/`, never `archived/`, so per-turn cost is flat regardless of archive size (T064). Tolerant load (unknown fields → WARNING, not a crash).
- **`managers/memory_manager.py`** — long-term memory: ChromaDB collections (`memory_{entity_id}` etc.), OpenAI embeddings per `config.ai_embedding_model` (default `text-embedding-3-large`), scope-filtered semantic recall. Under Feature 070 the per-chat collection holds one `daily_summary` record per (chat, calendar-date) — `collection_name_for_chat()` (`managers/memory_collections.py`) sanitizes a `@g.us`/`@c.us` chat id into a valid collection name (e.g. `120363…@g.us` → `memory_120363…_at_g.us`). Recall of `daily_summary` is still pure embedding similarity (Feature 077 will redesign this).
- **`managers/roll_marker_store.py`** (Feature 070) — claim-first SQLite store (`memory_rolls/roll_markers.db`, `PRIMARY KEY(chat, date)`): `claim()` inserts a `claimed` row (or reports the day already done), `commit()` flips it to `committed`. Makes the nightly roll idempotent and safe against crashes / concurrent startup sweeps.
- **`managers/summarizer.py`** (Feature 070) — the single day-summary prompt/call (`config.ai_model`), shared by the nightly roll and the offline migration backfill so both produce byte-identical summaries.
...
- **`managers/ledger_event_manager.py`** (Feature 033) — persists structured ledger events (fee agreements, bank deposits) recognized by the runtime constitution's "Ledger Event Recognition" rules, one immutable JSON file per event under `{data_root}/events/{event_id}.json` (`event_id` format: a source-type letter + `DDMMYY` + `HHMM` + a same-minute sequence digit, e.g. `A02022604480`) — moved out of `session.json` (Feature 024's original location) so events survive session archival/expiry independently. Called via `AIHandler`'s `capture_ledger_event` local tool wiring, never invoked directly by the model against Morning. **Querying (Feature 044)**: the same manager also answers natural-language questions over its own in-memory index (loaded at startup, kept current on every write) via `query_events(criteria: List[{text, hint}])` — a single fuzzy `text` search per criterion, each carrying an optional `hint` naming a closed group of likely-target fields (`identity`, `date`, `event_type`, `vat`, `amount`, `percentage`, `free_text`, `document`, `banking`), scored (never hard-filtered) via `rapidfuzz`. `AIHandler` exposes this as the RBAC-gated (godfather/admin only) `query_ledger_events` local tool; explicit OR/NOT/threshold reasoning and all aggregation (sums, owed-vs-received netting) are deliberately the model's own job over the raw returned events, never the tool's — there is no code-level ambiguity gate blocking on multiple name matches either (removed 2026-08-26; real candidates are always returned, and disambiguating is the model's judgment call, guided by `runtime_constitution.md`'s "Ambiguous names, OR, NOT, and threshold questions" section). `runtime_constitution.md` also establishes the ledger as a **cache over Morning, not a source of truth on its own**: a zero-match query result must be treated as a possible cache miss (backfill gap or sync-timing lag) rather than proof nothing exists, and the model is required to fall back to the corresponding Morning tool before reporting "not found" — except for `הסכם`/`בנק` events, which never exist in Morning at all.
- **`managers/reminder_manager.py`** + **`managers/pending_local_tool_approval_manager.py`** + **`services/reminder_delivery_service.py`** (Feature 054) — see "Reminders (Feature 054)" above.
- **`utils/whatsapp_audit_log.py`** — two-way audit logging (`log_inbound`/`log_outbound`), INFO level: every raw incoming Green API webhook event verbatim (binary fields like `jpegThumbnail` redacted) and every real outbound send, wired at each actual boundary-crossing call site in `denidin.py`/`whatsapp_handler.py`/`green_api_bot.py` (never centralized via monkey-patching, per the no-monkey-patching rule below).
- **`services/daily_summary_roll_service.py`** (Feature 070 — replaces the deleted `cleanup_service.py`) — one shared `APScheduler` `BackgroundScheduler` job (`CronTrigger`, 02:00 `Asia/Jerusalem`) that rolls every day newly aged out of the 14-day window: per (chat, date), claim a roll marker → summarize via `managers/summarizer.py` → write one `daily_summary` ChromaDB record → `Path.rename` those messages `messages/` → `archived/` → commit the marker. `run_startup_daily_roll_sweep` (21-day lookback) catches up on boot; empty days get a marker only, no summary. Never `unlink`s a message file.
...
- `logs/denidin.log` (production), `logs/test_logs/{test_file}.log` (per-test-file, auto-configured by `conftest.py`, app-level log output only — never pytest's own assertion/traceback text), and `logs/test_logs/pytest_results/*.txt` (pytest's complete, untruncated report for a single billed/expensive run, written by `scripts/run_single_test.sh`) — check these logs instead of re-running a billed/expensive test to get more diagnostic detail.

### Morning MCP integration (`apps/denidin-app` ↔ `apps/morning-mcp-app`)
Godfather/admin users manage invoices in natural Hebrew: `AIHandler` calls OpenAI's **Responses API** with the Morning server attached as a **remote MCP tool** (`type: "mcp"`, bearer-auth header), reached over a real ngrok tunnel — no local import of `denidin_mcp_morning` code, no mocking of the MCP round-trip. The tunnel URL is discovered via a per-environment shared status file (`shared/mcp-status-dev/`, `shared/mcp-status-prod/` — see "Environments (dev/prod)" above; must show `"status": "running"`); `apps/denidin-app`'s own `config.mcp.morning_status_file` points at its own environment's copy, never the other's. RBAC-gated: only godfather/admin roles get the tool attached. Current date is injected into `instructions` at reply time (Israel local, computed per call — not templated into the constitution file) so the model resolves relative dates correctly instead of guessing a wrong year.
