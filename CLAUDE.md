...
🚨 **When driving `scripts/run_multiple_billed_tests.sh`, SOUND OFF ON EACH TEST'S RESULT AS IT COMPLETES — never only in one summary once the whole sequence finishes — unless explicitly told otherwise for that run.** The script already announces each `[N/TOTAL] PASSED/FAILED:` result live, by design, specifically so this can be relayed as it happens; don't defeat that by backgrounding the call behind anything that buffers until the process exits (`| tail -N`, `| grep`, a `$(...)` capture, etc.) and reporting only at the end. Real incident (2026-08-26): an agent ran a 23-test sweep exactly that way — nothing streamed for several minutes, and the human had to explicitly demand a status ("SOUND OFF GODDAMNIT!!!") before getting anything. Poll the running script's output (or its per-test result files under `logs/test_logs/pytest_results/`) at a short interval — or use a `Monitor`-equivalent — and relay each new result the moment it appears. This applies with equal force to `scripts/run_single_test.sh` used in a loop for the same purpose.
