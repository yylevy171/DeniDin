...
- 🚨 **Deploying a cut release is done ONLY by running `scripts/deploy_release.sh` directly — no manual pre-flight checks, no ad-hoc connectivity probes (`tailscale status`, manual `ssh`, etc.) beforehand.** The script performs its own connectivity/health verification internally and fails loudly with a clear error if something's wrong — a manual check first is redundant busywork, not caution, and the user has explicitly objected to it ("use the goddamn scripts"). This applies to `dev` and `prod` alike; the read-only Windows-prod operational scripts (`scripts/windows_prod/*.sh`) remain fine to use for their own documented purpose (checking status/logs), just not as a substitute pre-check before a deploy.
