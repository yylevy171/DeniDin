...
**`prod` for both apps runs on a dedicated, always-on Windows laptop (Feature 035)**, reachable from the Mac over Tailscale/SSH — never a local host process and never the machine this session is running on. Day-to-day operation (start/stop, deploy, read logs, browse the read-only `data` mount) is done entirely from the Mac; see `specs/035-windows-always-on-prod/quickstart.md` for the full runbook and `scripts/windows_prod/*.sh` for the operational scripts (`verify_windows_prod.sh` for read-only connectivity/health checks, `deploy_and_verify.sh` for build-once-on-Mac-and-ship deploys, `verify_reboot_recovery.sh` for the one deliberately disruptive reboot-recovery check). Every start/stop/deploy against this box is still subject to the same per-action approval rule as any other environment start (see "NEVER START AN ENVIRONMENT... WITHOUT EXPLICIT APPROVAL" above).
