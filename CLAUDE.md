...
- **Data** (`sessions/`, `events/`, `memory/`, `media/`): already mounted read-only via sshfs/macFUSE at `~/denidin-winprod-data` (see `scripts/windows_prod/mount_data.sh` §9a of the quickstart — one-time setup, persists across reboots via `reconnect`). If not mounted, run that script with the SSH host alias (`denidin-winprod` in `~/.ssh/config`) — do **not** open an ad-hoc `ssh`/manual mount instead, use the existing sanctioned mount/script.
