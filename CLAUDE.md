...
`expensive` are new.
- New/updated specs belong under `specs/in-progress/` (drafting/pre-clarification through active implementation) or `specs/backlog/` (post-clarification, not currently being worked — replaces the old `specs/P0/`/`P1/`/`P2/` split as of 2026-07-21; priority is now tracked via each spec's own `Priority` field, not by folder); `specs/done/` and `specs/obsolete/` are historical archives — never delete from them (`specs/obsolete/` also replaces the old `specs/not-doing/`, and now covers specs whose described issue no longer applies against current code, not just cancelled features). Bugfix specs go in `specs/bugfixes/bugfix-###-description.md` while open, and move to `specs/done/` (flat, no subfolder — see below) or `specs/obsolete/bugfixes/` once resolved or found stale. `specs/not_reproducible/bugfixes/` (added 2026-07-21) is a fourth closure destination, distinct from both: use it for a bug where a root cause was investigated and a human decision was made to close it, but nothing was actually fixed — e.g. the behavior is accepted as inherent/non-deterministic model risk rather than an app-level bug (see `bugfix-013` for the first example). Don't conflate with `done/` (implies a fix landed) or `obsolete/` (implies the issue no longer applies or was rejected outright).
- **`specs/done/` is organized by release version (2026-08-20 reorganization)**: `specs/done/vX.Y.Z/<NNN-feature-name>/` and `specs/done/vX.Y.Z/bugfix-NNN-description.md` sit side by side under the same version folder — no separate `features`/`bugfixes` split within a version. Everything that predates Feature 034 (versioning itself, introduced 2026-08-02) has no real recorded version and is best-effort bucketed under `specs/done/v0.0.1/` (the first-ever cut) rather than left unversioned. A freshly-finished feature or bugfix (haleluya, or a plain `mv` per `specs/bugfixes/README.md`) still lands **flat** at `specs/done/<name>` with no version folder — `scripts/cut_release.sh` is what sweeps every flat entry into a newly-created `specs/done/vX.Y.Z/` the next time a release for that app is actually cut, so a spec's version folder always reflects a real, already-cut release, never a projected one.
