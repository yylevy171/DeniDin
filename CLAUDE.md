...
- **Never run a release-cutting or rollback script (`scripts/cut_release.sh`, `scripts/rollback_release.sh`) on your own initiative, and never infer/guess a target version "the previous one" even when it seems obvious from context.** Both scripts may only be invoked with an app (and, for rollback, an environment) and an exact version explicitly supplied by the human in that specific request. Approval for one release or rollback never carries over to the next — same rule as every other "ask every time" gate in this document.
